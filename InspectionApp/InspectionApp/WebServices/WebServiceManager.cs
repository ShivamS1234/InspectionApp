...
namespace InspectionApp.WebServices
{
  public class WebServiceManager : ServiceConfigrations
  {
    ...
    public async Task<WebServiceResult<CommonResponseDTO>> GetAllDatabyCmpIdAsync(CommonRequestDTO companiesRequestDTO)
    {
      UTF8Encoding encoder = new UTF8Encoding();
      byte[] data = encoder.GetBytes(JsonConvert.SerializeObject(companiesRequestDTO));
      string utfString = Encoding.UTF8.GetString(data, 0, data.Length);

      var result = await webService.PostAsync<CommonResponseDTO>("AllData/GetAllData", utfString);
      return result;
    }
    ...
    public async Task<WebServiceResult<InspectionDetailsResponseDTO>> GetHeaderDetailsAll()
    {
      ...
      var result = await webService.GetAsync<InspectionDetailsResponseDTO>("InspectionDetails/GetAllInspectionDetails", "");
      return result;
    }
