    ...
    class InspectionListViewModel : BaseViewModel
    {
                    ...
                    if (_deleteVolumeCommand == null)
                    {
                        _deleteVolumeCommand = new Command(async (e) =>
                        {
                            ...
                            if (status)
                            {
                                ...
                                await InitData.HeaderRepo.DeleteItemAsync(X => X.Id == item.Id);
                                await InitData.DetailsRepo.DeleteItemAsync(x => x.InspectionHeaderId == item.Id);
                            }
                        });
                    }
        ...
        public async void GetAllInitData()
        {
            try
            {
                ...
                //};
                var result = await InitData.HeaderRepo.Get<Model.InspectionHeaderTable>();
                if (result != null)
                {
                    if (result.Count > 0)
                    {
                        ...
                        {
                            LstInspectionHeaderModel.Add(new InspectionHeaderModel
                            {
                                Id = (int)data.Id,
