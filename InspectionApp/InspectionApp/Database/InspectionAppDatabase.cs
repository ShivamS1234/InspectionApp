...
{
    public class InspectionAppDatabase<T> : IDatabaseService<T> where T : Entity, new()
    {
        #region propertise
        ...
        public async static Task SaveCompanies(IList<Company> Companys)
        {
            ...
            {
                var result = _connection.Table<Company>().Where(x => x.Id == account.Id).ToList();
        ...
        public void BulkSaveCompanies(IList<Company> Companys)
        {
            try
            {
                ...
                //    Count = group.Count()
                //}).Where(x => x.Count > 1);
                var data = _connection.Table<Company>().ToList();
        ...
        public async Task InsertAll(List<T> instance)
        {
            try
            {
                //var gropdata = Accounts.GroupBy(info => info.AccountId).Select(group => new
